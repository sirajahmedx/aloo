          ...
          return (
            ...
            >
              <blockquote className="flex h-full flex-col justify-between gap-5 pointer-events-none">
                <span className="relative z-20 block font-serif text-[15px] font-medium italic leading-[1.88] tracking-[0.012em] text-[#E3D7BE] transition-colors duration-700 group-hover:text-[#F1E8D6] sm:text-[17px]">
                  &ldquo;{item.quote}&rdquo;
                </span>
                {item.name && (
                  <footer className="mt-auto flex flex-col gap-1 border-t border-[#2F4F4F]/25 pt-4">
                    <span className="font-serif text-[14px] font-semibold tracking-[0.03em] text-[#D8CAAD] sm:text-[15px]">
                      {item.name}
                    </span>
                    {item.title && (
                      <span className="font-sans text-[11px] font-light tracking-[0.06em] text-[#2F4F4F] uppercase sm:text-[12px]">
                        {item.title}
                      </span>
                    )}
                  </footer>
